    ...
    return seasons
        ...
        .map(season => ({
            ...
            episodes: Array.isArray(season.episodes)
                ? season.episodes.slice().sort((a, b) => Number(a.number) - Number(b.number))
                : [],
            episodeCount: Array.isArray(season.episodes) ? season.episodes.length : 0,
            playableEpisodeCount: Array.isArray(season.episodes)
                ? season.episodes.filter(hasRealSource).length
                : 0
        }))
